/*
    ...
    Example: (_this#1) call HR_Garage_fnc_reciveBroadcast;
...
//set the new data
if (_switch) then { [getPlayerUID _player] call HR_Garage_fnc_releaseAllVehicles };
...
if (_isPlayer) then {
    if (_switch) then {
        ...
        HR_Garage_SelectedVehicles = [_catIndex, _vehUID, _vehicle#1];
        [] call HR_Garage_fnc_reloadPreview;
        if (
            HR_Garage_Pylons_Enabled //Pylon editing enabled
            && { HR_Garage_hasAmmoSource } //or ammo source registered
        ) then { [] call HR_Garage_fnc_reloadPylons };
    };
    [true] call HR_Garage_fnc_toggleConfirmBttn;
};
[_isPlayer] call HR_Garage_fnc_reloadExtras;

//refresh the category display
private _disp = findDisplay HR_Garage_IDD_Garage;
private _ctrl = HR_Garage_Cats#_catIndex;
Trace_2("r.Broadcast - Ctrl: %1 | Active: %2" , _ctrl, ctrlEnabled _ctrl);
if (ctrlEnabled _ctrl) then {
    [_ctrl, _catIndex] call HR_Garage_fnc_reloadCategory;
};
