/*
    ...
    Example: [getPlayerUID player, _index, _newIconIndex, clientOwner, player] remoteExecCall ["HR_Garage_fnc_findMount",2];
...
private _failed = { ["STR_HR_Garage_Feedback_requestMount_Denied"] remoteExec ["HR_Garage_fnc_Hint", _owner]; [true] remoteExecCall ["HR_Garage_fnc_toggleConfirmBttn", _owner]; false };
...
if (
    ...
    && !(_player isEqualTo (_player call HR_Garage_cmdClient))       //cmd overwrite
) exitWith _failed;
if !((_mount#3) in ["", _UID]) exitWith _failed; //Checked out by someone else

//check out mount
_mount set [3, _CheckedUID];
[nil, _CheckedUID, 4, _vehUID, _player, false] call HR_Garage_fnc_broadcast;
true;
