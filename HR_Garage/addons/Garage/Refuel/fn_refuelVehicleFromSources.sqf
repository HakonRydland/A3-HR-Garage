...
while {count (HR_Garage_Sources#1) > 0} do {
    if (_neededCapacity == 0) exitWith {};
    Trace_1("Needed capacity: %1", _neededCapacity);

    private _sourceUID = HR_Garage_Sources#1#0;
    private "_sourceData";
    for "_i" from 0 to 4 do { _sourceData = (HR_Garage_Vehicles#_i) get _sourceUID; if (!isNil "_sourceData") exitWith {}; }; //find vehicles in categorys, typically cat 0 "cars"
    ...
    private _fuelCargo = if (HR_hasAce) then {
        _fuelData # 2;
    } else {
    ...
    if (_fuelCargo < _neededCapacity) then {
        ...
        _fuelData set [if (HR_hasAce) then {2} else {1}, 0];
        (HR_Garage_Sources#1) deleteAt ((HR_Garage_Sources#1) find _sourceUID);
        _sourceEmptied = true;
    } else {
        if (HR_hasAce) then {
...
{ _x call HR_Garage_fnc_broadcastStateUpdate; } forEach _stateChanges;
if (_sourceEmptied) then { [1] call HR_Garage_fnc_declairSources; };
